...
describe("Testing Group display", () => {
  ...
  test("testing group display correct contacts", () => {
    render(<GroupComponent contact={contact}/>);

    const groupComponentElement = screen.getByTitle("calendar-title");
    expect(groupComponentElement).toBeInTheDocument();
    expect(groupComponentElement).toHaveTextContent("September");
    expect(groupComponentElement).toHaveTextContent("2021");
  });
