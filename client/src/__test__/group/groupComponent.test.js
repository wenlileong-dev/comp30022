...
describe("Testing Group display", () => {
  let contact = {
    ...
    address: "",
    remark: "remark",
    groupID: "615300aff44c8461f8432d7f",
  };
  const renderGroupComponent = () => {
    return (
      <Router>
        <GroupComponent contact={contact} />
      </Router>
    );
  };
  test("testing group display correct contacts", () => {
    render(renderGroupComponent());
    const groupComponentElement = screen.getByTestId("groupComponent");
    expect(groupComponentElement).toBeInTheDocument();
    expect(groupComponentElement).toHaveTextContent(`${contact.firstName}`);
    expect(groupComponentElement).toHaveTextContent(`${contact.lastName}`);
    expect(groupComponentElement).toHaveTextContent(`${contact.remark}`);
  });
});
