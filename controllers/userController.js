...
const userPostRegister = async (req, res) =>{
    try{
        User.findOne({ email: req.body.email }).then((user) => {
            ...
            if(user){
                res.status(200).json({ success: false, error: 'Email already registered'});
            }else{
                ...
                if(reg.test(req.body.password)){
                    ...
                    res.status(200).json({ success: false, error: 'Password not valid!'});
                }
            }
        })
    }catch{
                ...
                bcrypt.hash(req.body.password,salt,(err,hash)=>{
                    ...
                    User.findOne({email:req.body.email}, function (err,duplicateUser){
                        if(duplicateUser){if(duplicateUser._id != req.params.id) {
                            ...
                            res.status(409).json({success: false, message:"another User has already registered that email"})
                        }}
                        else{
                            User.findOneAndUpdate(
                                ...
                                function(err,updateUser){
                                    if(err){
                                        res.status(404).json({success: false, message:"User account does not exist"})
                                    }else{
